...
import ru.mts.workflowscheduler.share.validation.JsonParseResult;
...
public class ValidationServiceImpl implements ValidationService {
  ...
  public ValidationResult validate(String toValidate, BaseSchema schema){
    ...
    if(jsonNode != null) {
      Context ctx = new Context(errorCompiler, scriptExecutorService);
      schema.validate(ctx, jsonNode);
      List<ErrorDescription> validationErrors = schema.getViolations().stream()
          .map(errorCompiler::toErrorDescription).flatMap(l -> l.stream()).toList();
