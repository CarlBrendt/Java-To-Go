...
set rabbitmq_details = t.new_details
from (select id,
      ...
      from (select remote_data.*,
            ...
            from dblink('dbname=${RABBITMQ_DB_NAME} user=${RABBITMQ_DB_USERNAME} password=${RABBITMQ_DB_PASSWORD}',
                        'select distinct on (name, tenant_id) id, create_time, change_time, description, name, tenant_id, actual_status, desired_status, start_workflow_id, rabbitmq_starter_details, virtual_host, user_name, user_pass, addresses, queue from ${RABBITMQ_DB_SCHEMA}.rabbitmq_starter')
