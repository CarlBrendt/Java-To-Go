...
import static ru.mts.ip.workflow.engine.validation.Constraint.VALID_WORKFLOW_EXPRESSION;
...
public class WorkflowDefinitionSchemaFromDraft extends ObjectSchema{
  ...
  public WorkflowDefinitionSchemaFromDraft(Constraint ...constraints) {
    super(constraints);
    ...
    putField(CREATE_TIME, new BaseSchema());
    putField(CHANGE_TIME, new BaseSchema());
    putField(DELETED, new BaseSchema());
    putField(ID, new BaseSchema());
    putField(VERSION, new BaseSchema());
    putField(STATUS, new BaseSchema());
  }
        ...
        if(WorkflowType.POSIBLE_VALUES.contains(type)) {
          if(Const.WorkflowType.COMPLEX.equals(type)) {
            return new ObjectSchema(Map.of(
                COMPILED, new ObjectSchema(FILLED, VALID_WORKFLOW_EXPRESSION),
                DETAILS, new WorkflowDetailsSchema(null)
            ));
          } else {
